...
def check_names(names_set):
    regex_dict = {".+’":("’", ""), ".+!":("!", ""), "[Tt]he\s.+":("[Tt]he\s", ""), "[oO]\s.+": ("[Oo]\s", ""), ".+['’]s": ("['’]s", ""), ".+\s['’]s": ("\s['’]s", ""), "[Pp]rince\s.+": ("[Pp]rince\s", ""), "[Pp]rincess\s.+": ("[Pp]rincess\s", ""), "[Kk]ing\s.+": ("[Kk]ing\s", ""), "[Qq]ueen\s.+": ("[Qq]ueen\s", ""), "[Dd]ame\s.+": ("[Dd]ame\s", ""), "[Ll]ady\s.+": ("[Ll]ady\s", ""), ".+['’,.]": ("['’,.]", ""), "[Dd]oes\s.+": ("[Dd]oes\s", "")}
    characters_set = set()
