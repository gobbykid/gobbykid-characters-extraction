...
def gender_recognition(names_set):
    ...
    for name in single_word_names:                      #_______________________________________NEW BLOCK FOR THE BRANCH
        ...
        elif name in already_classified_female_characters: #If the name is already classified as female character, it is added to the list of female characters
            female_characters.append(name)              #_______________________________________END OF THE NEW BLOCK FOR THE BRANCH
        else:                                            #check by means of genderComputer on the list of names composed by a single word
    ...
    for full_name in multiple_words_names:                        
        joined_name = " ".join(full_name)

        if joined_name in already_classified_male_characters: #_______________________________________NEW BLOCK FOR THE BRANCH
            male_characters.append(joined_name)
        elif joined_name in already_classified_female_characters:
            female_characters.append(joined_name)              #_______________________________________END OF THE NEW BLOCK FOR THE BRANCH
        else:
                                                                #The block of checks aims at guessing the gender on the basis of genderdized words present in the strings without using any additional library
                        ...
                        if already_added:
                            ...
                            if single_name in male_characters:
                                ...
                                if name_gender == "male":
                                    ...
                                    if full_name.index(single_name) == len(full_name) -1: #it is checked if the name is the last word of the string because Genderize works only in the first iteration since it takes in input the full name
